...
function mapToolbarHandler(btn, evt) {
    ...
    if (btn.id == "PrintMap") {
        if (btn.pressed) {
            ...
            // BEGIN SOGIS bei Massstabsbeschränkungen
            var combobox = Ext.getCmp('PrintScaleCombobox');
            var record = combobox.findRecord('value', combobox.value);
            var index = combobox.store.indexOf(record);
            combobox.fireEvent('select', combobox, record, index);
            // END SOGIS
...
/* SOGIS: This function has been reimplemented in sogis.js */
...
function exportBoxDetailFromMap() {
    // get all visible layers including WMTS layers
    ...
    var url = wmsURI + 'SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&EXCEPTIONS=INIMAGE&FORMAT=image%2F'+exportBoxValues.format+'&TRANSPARENT='+exportBoxValues.transparent+'&STYLES=&CRS='+authid+'&DPI='+exportBoxValues.dpi+'&LAYERS='+encodeURIComponent(layers)+'&WIDTH='+exportBoxValues.width+'&HEIGHT='+exportBoxValues.height+'&BBOX='+exportLayer.features[0].geometry.bounds.toBBOX(1,thematicLayer.reverseAxisOrder());
